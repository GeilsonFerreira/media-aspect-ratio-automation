...
class MediaMonitorHandler(FileSystemEventHandler):
    ...
    def _handle_video(self, file_path: Path):
        try:
            ...
            if output_path.exists():
                logger.info(
                    f"Vídeo já processado: {output_path.name}"
                )
                return
            ...
            logger.info(
                f"Vídeo finalizado: {output_path.name}"
            )
    ...
    def _handle_image(self, file_path: Path):
        try:
            ...
            if output_path.exists():
                logger.info(
                    f"Imagem já processada: {output_path.name}"
                )
                return
            ...
            logger.info(
                f"Imagem convertida para vídeo: {output_path.name}"
            )
